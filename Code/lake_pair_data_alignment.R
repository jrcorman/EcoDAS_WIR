...
dperi <- dgraph(peri, "Perimeter (units)", "no")
dperi

# Elevation
el <- pair("Data/NLA2007_lakes.csv", "ELEV_PT")
del <- dgraph(el, "Elevation (units)", "yes")
del

# Max Depth
depth <- pair("Data/NLA2007_lakes.csv", "DEPTHMAX")
ddepth <- dgraph(depth, "Depth (units)", "no")
str(depth)
boxplot(depth$avg ~ depth$type)

# All density plots as one
library(gridExtra)
grid.arrange(del, dCALA, dLA, dperi, ddepth, dsec, dtemp_1m, dtemp_bottom, ncol=2)
